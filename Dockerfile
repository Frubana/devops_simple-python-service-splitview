...
ARG SONARQUBE_URL
ARG SONARQUBE_ENV
RUN if [ $SONARQUBE_ENV = "develop" ] ; then sonar-scanner -Dsonar.host.url=$SONARQUBE_URL -Dsonar.login=$SONARQUBE_TOKEN ; else echo "$SONARQUBE_ENV no envia a sonar"; fi
